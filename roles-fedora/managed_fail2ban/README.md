...
Ansible Role zur Verwaltung des fail2ban Services.

Default Variables
```yaml
managed_fail2ban_enabled: true
managed_fail2ban_config:
  loglevel: "WARNING"
...
managed_fail2ban_jail_config:
  ...
  action: '%(action_)s'
managed_fail2ban_jail_services:
  sshd: |
    port      = ssh
    logpath   = %(sshd_log)s
